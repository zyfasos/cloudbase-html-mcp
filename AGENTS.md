...
- 版本准备和发布按 [发布维护指南](docs/releasing.md) 执行：使用 `release:version` 同步当前版本及命令，使用 `release:pack` 生成并核验实际 tgz，授权发布后用 `release:verify` 核对公共包和目标标签。不得全仓库替换历史版本，不把旧 CI/桌面验收结论套到新版本；发布验证过的同一 tgz，包内内容变化后重新核验。
