...
源码已在 [GitHub](https://github.com/zyfasos/cloudbase-html-mcp) 公开，采用 MIT；第三方依赖保留各自许可证。`cloudbase-html-mcp@0.4.0-beta.1` 已在 npm 公开发布；当前 `beta` 和 `latest` 均指向该测试版，接入模板继续固定具体版本。GitHub 远端尚未同步本轮改动。

业务 HTML、私人环境信息、凭据、本地登记和 docs/implementation/ 不进入 Git（包括历史）；实施档案只在本地维护。

## v0.4 验证与发布安排

macOS 本地已在 Node 22.20.0 和 24.20.0 分别通过 check 与全量 122 项离线测试，包含真实 tarball 安装、合成前序版本升级及 STDIO 六工具生命周期（云端替身）；新增 macOS/Windows、Node 22/24 CI 矩阵，工作流尚未推送执行。四款客户端的版本和输入界面已记录在客户端指南，实际安装及 Windows 实机仍待验证。macOS Node 22.20.0 已通过公共 npm 全新缓存下载安装、版本查询、真实 STDIO 六工具发现及缺失配置诊断；在线版本化指南与发布包内容一致。随后完成客户端及指定文件/环境的云端验收；验收通过并明确授权后发布 0.4.0。不得将模板或截图当作兼容性验收。
