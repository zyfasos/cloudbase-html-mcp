...
本地 macOS Node 22.20.0、24.20.0 各通过 `npm run check` 和 144 项离线测试（绑定修复新增 10 项、目录地址新增 8 项、恢复提示复审新增 2 项，0 失败/跳过），包含真实 STDIO 子进程、进程退出恢复、实际 tarball 安装和重装。云端均为替身，未改真实登记或站点。这些修复纳入 0.4.0-beta.3，未回写 beta.2。beta.3 尚未执行 Windows CI 或桌面实测；与上方 beta.2 的 CI、桌面证据分别记录。新版仍需在桌面客户端复验目录访问、原站点恢复和更新。
