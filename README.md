...
```mermaid
flowchart LR
    A["本地 HTML<br/>报告 · 演示 · 交互页面"] -->|"首次发布 / 更新在线页面<br/>publish_html"| B["已部署页面<br/>分享链接"]
    B -->|"下线 offline_html<br/>删除云端内容"| C["离线登记<br/>保留站点身份"]
    C -->|"指定本地文件恢复<br/>online_html"| B
```
...
- 上传单份非空 UTF-8 `.html`/`.htm` 文件，最多 5 MiB；关联的本地图片、CSS、JS 不会一起上传，发布前应确认页面可独立使用。
