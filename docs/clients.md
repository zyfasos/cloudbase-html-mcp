...
本指南说明如何手动添加本地 STDIO MCP。当前 v0.4 为 **0.4.0-beta.2 公开测试版**，WorkBuddy 与千问办公的 macOS 接入和发布已获用户实测确认，WorkBuddy 下线也已通过；同 URL 更新及其他客户端仍待验证；恢复实测暴露了共享文件绑定问题，见下方记录。也可按 [快速开始](getting-started.md#install-from-source-or-a-local-package) 使用本地包或已取得的 v0.4 源码。
...
当前源码已修复显式目标与文件默认绑定混用的问题，并补充离线、真实 STDIO 子进程及实际安装包回归：恢复 A 保留 B，更新 A 也保持原 URL。修复尚未发布，需升级后的桌面实测再次验收；不要将本地替身测试算作真实云端通过。
