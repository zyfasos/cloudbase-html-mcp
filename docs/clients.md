...
这是一份接入指南，兼容性结论以实际验证为准。此前 beta.3 的 macOS/Windows 运行时 CI 矩阵与公共 npm 冷启动已通过；当前 beta.4 的验证状态单独记录，结果见 [项目验证状态](../PROJECT.md#v04-验证与发布安排)；Windows 桌面接入、QoderWork 和豆包工作仍待实测。不同版本可能提供不同控件，验证后按实际版本更新本表。
