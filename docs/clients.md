...
同一启动程序不意味着同一种客户端 JSON。以下新增 CLI、IDE、OpenClaw 与 Hermes 示例依据 2026-09-11 官方文档整理，仅验证示例结构与命令，尚未通过本项目真实客户端验收。已有桌面实测按当时版本保留在 [CHANGELOG 的桌面实测记录](../CHANGELOG.md#桌面实测记录)，不自动升级为当前版通过。
...
这是一份接入指南，兼容性结论以实际验证为准。运行时 CI 与公共 npm 冷启动结果见 [项目验证状态](../PROJECT.md#v05-release)；桌面实测的证据表与边界统一记录在 [CHANGELOG 的桌面实测记录](../CHANGELOG.md#桌面实测记录)（接入、发布/下线已通过；同 URL 更新、重启后查询与原 URL 恢复仍待验收；Windows 桌面实机按用户决定暂缓）。不同版本可能提供不同控件，验证后按实际版本更新本表。
