...
当前 v0.4 为 **0.4.0-beta.5 测试版**，将单 HTML 上限提高到 20 MiB，并包含此前的 BOM、CLI 提示、共享文件绑定和目录分享地址修复。请使用文中的固定版本命令，也可采用 [源码或本地安装包](#install-from-source-or-a-local-package)。macOS 和 Windows 均要求 Node.js 22+，安装包不内置 Node。发布与验证进度单独记录；WorkBuddy/千问办公桌面证据来自 beta.2，新版桌面复验仍待完成，见 [验证状态](../PROJECT.md#v04-验证与发布安排)。
