...
export function createServer(factory = () => {
  ...
  server.registerTool('publish_html', {
    description: '将用户指定的 HTML 首次发布或覆盖在线页面，仅写当前对象，不创建快照。更新同一 URL 时先 get_html，复用 siteId 或经当前环境路由校验的 siteUrl，并传查询返回的 sha256。离线页面须显式 online_html 恢复；此操作会公开 HTML。域名映射不变时更新 URL 不变。',
    inputSchema: {
      localPath: z.string().min(1).describe('用户指定的本地 .html/.htm 文件绝对路径；非空有效 UTF-8，最多 5 MiB。只上传此文件，关联资源不上传。更新时仍须提供新内容所在的路径。'),
      ...
      newPage: z.boolean().optional().describe('默认 false。仅在用户明确要求另建页面时设为 true，不能同时传 siteId 或 siteUrl；新页验证成功后替换该路径的本地登记，旧云端页面保留。持续更新同一 URL 时不要设为 true。'),
    },
  ...
  server.registerTool('offline_html', {
    description: '下线指定页面：永久删除当前云端 HTML 及该站点旧版快照，保留本地登记。必须用户明确要求；先 get_html 查询哈希。先核对 COS 桶版本控制，启用、暂停或无法确认时拒绝删除。下线不清除外部浏览器/CDN 缓存；清理不完整必须按返回建议恢复。',
    inputSchema: {
  ...
  server.registerTool('online_html', {
    ...
    inputSchema: {
      siteId: siteIdField, siteUrl: siteUrlField,
      localPath: z.string().min(1).describe('本次用户明确指定的 .html/.htm 绝对路径，UTF-8、非空、最多 5 MiB；不静默选用旧文件。siteId、siteUrl 二选一另行提供。'),
    },
    annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: true },
