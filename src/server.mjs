...
export function createServer(factory = () => {
  ...
  const displayNameField = z.string().optional().describe('可选的本地站点名称：去除首尾空白后1–120个Unicode字符，不含控制字符。省略保留原名；不支持清空或独立改名。名称可重复，不是写操作选择器。');
  server.registerTool('publish_html', {
    description: '将用户指定的 HTML 首次发布或覆盖在线页面，仅写当前对象，不创建快照。更新同一 URL 时先 get_html，复用 siteId 或经当前环境路由校验的 siteUrl，并传查询返回的 sha256。离线页面须显式 online_html 恢复；此操作会公开 HTML。默认返回 /sites/<siteId>/ 分享地址并验证目录响应；云端仍保存 index.html，旧完整文件 URL 继续可作为目标。域名映射不变时更新 URL 不变。返回资源诊断resourceDiagnostics和标题/名称；诊断不阻断发布，不自动上传或修复关联资源。',
    inputSchema: {
  ...
  server.registerTool('list_html', {
    ...
    inputSchema: {
      query: z.string().optional().describe('最多200个Unicode字符的关键词；NFKC及大小写归一化，空白分词AND匹配本地名称、HTML标题或来源文件名。空白等同无搜索；不搜索完整目录或HTML，不访问云端。结果可能重名，先确认ID/URL再写入。'),
  ...
  server.registerTool('online_html', {
    description: '将已登记的离线页面重新公开上线，读取本次用户指定的 HTML 并恢复原 siteId 路径，不写快照。云端须不存在；已在线时使用 publish_html。此前上线超时但已写入相同内容可验证完成。要求启用本地目录。返回本次资源诊断与标题，诊断不阻断发布；省略displayName保留原名称。',
    inputSchema: {
      siteId: siteIdField, siteUrl: siteUrlField, displayName: displayNameField,
